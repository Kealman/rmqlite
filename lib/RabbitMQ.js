  ...
  class RabbitMQ {
    ...
    publish(message, routingKey) {
      return co(function *() {
        ...
        return ch.publish(this.exchanger, routingKey || this.queue, new Buffer(JSON.stringify(message)));
      }.bind(this));
    }
    ...
    _bindProccess(method, routingKeys) {
      ...
      return co(function *() {
        ...
        for (let route of routingKeys) {
          ...
          if (bind) {
            this.binds = this.binds.push(route);
          } else {
            this.binds.splice(this.bind.indexOf(route), 1);
          }
        }
      }.bind(this));
    }
